...
# history_file_path = 'history_tasks/task_history.json'
...
#         with open('prompts/instruction.txt', 'w', encoding='utf-8') as file:
#             file.write(content)
...
from datetime import datetime
import json
from . import execute_LLM_plan # Relative import
from .llm_as_planner import generate_and_correct_plan, insert_code_into_file, TARGET_TASK_FUNCTIONS_PATH, GENERATED_FUNCTION_NAME_PATH # Relative import

history_file_path = 'history_tasks/task_history.json'
similarity_flag_path = 'logs/similarity_flag.json'
task_description_file_path = 'logs/task_description.json'
short_term_memory_path = 'memory/memory3.json'
...
def save_task():
    ...
    if task:
        ...
        # Generate and correct plan
        print("Generating and correcting plan...")
        final_plan_code, feedback_output, func_name = generate_and_correct_plan(task)
        
        # Print LLM feedback to terminal
        print("\n--- LLM Feedback from Planner ---")
        print(feedback_output)
        print("-----------------------------------")
        
        # Write the final plan to task_functions.py
        insert_code_into_file(final_plan_code, TARGET_TASK_FUNCTIONS_PATH, 6)
        
        # Save the generated function name
        with open(GENERATED_FUNCTION_NAME_PATH, 'w') as file:
            json.dump({"function_name": func_name}, file)
        
        messagebox.showinfo("Success", "Plan generated and corrected. Starting task!")
        
        # execute_LLM_plan.run_scripts() # This is no longer needed as planning is integrated
        
        # Parse and execute the task (which will use the newly written task_functions.py)
        execute_LLM_plan.parse_and_execute_task(robots[0])
    else:
        messagebox.showwarning("Input Error", "Please enter a task.")
