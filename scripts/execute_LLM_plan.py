...
# Replace with your actual OpenAI API key if different, or keep consistent with llm_as_planner.py
api_key = "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"

# sys.path.append('logs')
task_queue = queue.Queue()


directory_path = 'memory/short_term'
task_description_file_path = 'logs/task_description.json'
results_file_path = 'memory/analysis_results.json'
...
#     # 尝试读取现有的JSON文件
#     results_file_path = os.path.join(directory, 'memory/analysis_results.json')
...
# Write the extracted sentences to prompts/long_term_memory.txt
with open(os.path.join('prompts', 'long_term_memory.txt'), 'w', encoding='utf-8') as f:
    for sentence in sentences:
        f.write(sentence + '\n')
        ...
        if len(action_queue) > 0:
            try:
                ...
                if act['action'] == 'ObjectNavExpertAction':
                    ...
                    compare_objects_location('memory/objects_locations1.json', 'memory/objects_locations2.json', 'memory/memory3.json')
...
def GoToObject(robot, dest_obj):
    ...
    # 1. Try to find object in long-term memory
    try:
        with open('memory/longterm_memory.json', 'r') as f:
            longterm_memory = json.load(f)
        
        for region_center_str, objects_in_region in longterm_memory.items():
            for obj_data in objects_in_region:
                if re.match(dest_obj, obj_data['objectType']):
                    dest_obj_pos = [obj_data['position']['x'], obj_data['position']['y'], obj_data['position']['z']]
                    dest_obj_id = obj_data['objectType'] # Use objectType as ID for consistency
                    break
            if dest_obj_pos:
                break
    except FileNotFoundError:
        print("Warning: longterm_memory.json not found. Proceeding without long-term memory.")
    except Exception as e:
        print(f"Error loading or parsing longterm_memory.json: {e}. Proceeding without long-term memory.")

    # 2. If not found in long-term memory, or if long-term memory loading failed, fall back to visible objects
    if dest_obj_pos is None:
        objs = list([obj["objectId"] for obj in c.last_event.metadata["objects"]])
        objs_center = list([obj["axisAlignedBoundingBox"]["center"] for obj in c.last_event.metadata["objects"]])
        ...
        for idx, obj in enumerate(objs):
            ...
            if match is not None:
                dest_obj_id = obj
                center = objs_center[idx]
                dest_obj_pos = [center['x'], center['y'], center['z']]
                break
    
    if dest_obj_pos is None:
        raise ValueError(f"Destination object '{dest_obj}' not found in long-term memory or visible objects.")
...
def ExploreObject(robots, dest_obj, dest_obj2):
    ...
    print ("Reached: ", dest_obj)
    return exit_goto
def GoToObject_next_time(robots, dest_obj, json_file='memory/objects_locations.json', json_file2='memory/objects_locations2.json'):
    print ("Going to ", dest_obj)
    # check if robots is a list
    ...
    return reach_flag
def GoToObject_with_memory(robots, dest_obj, json_file='memory/memory3.json'):
    print ("Going to ", dest_obj)
    # check if robots is a list
...
def PickupObject(robot, pick_obj):
    print(f"Attempting to pick up: {pick_obj}")
    robot_name = robot['name']
    agent_id = int(robot_name[-1]) - 1
    ...
    if pick_obj_id is None:
        print(f"Error: Object '{pick_obj}' not found for pickup.")
        return
    
    action_queue.append({'action':'PickupObject', 'objectId':pick_obj_id, 'agent_id':agent_id})
    print(f"Successfully picked up: {pick_obj}")
    
def PutObject(robot, put_obj, recp):
    print(f"Attempting to put {put_obj} on/in: {recp}")
    robot_name = robot['name']
    agent_id = int(robot_name[-1]) - 1
    objs = list(set([obj["objectId"] for obj in c.last_event.metadata["objects"]]))
    objs_center = list([obj["axisAlignedBoundingBox"]["center"] for obj in c.last_event.metadata["objects"]])
    ...
    if recp_obj_id is None:
        print(f"Error: Receptacle '{recp}' not found for putting object.")
        return
        
    action_queue.append({'action':'PutObject', 'objectId':recp_obj_id, 'agent_id':agent_id})
    print(f"Successfully put {put_obj} on/in: {recp}")
         
def SwitchOn(robot, sw_obj):
    print(f"Attempting to switch on: {sw_obj}")
    robot_name = robot['name']
    agent_id = int(robot_name[-1]) - 1
    objs = list(set([obj["objectId"] for obj in c.last_event.metadata["objects"]]))
    ...
    if sw_obj_id is None:
        print(f"Error: Object '{sw_obj}' not found for switching on.")
        return
    
    action_queue.append({'action':'ToggleObjectOn', 'objectId':sw_obj_id, 'agent_id':agent_id})
    print(f"Successfully switched on: {sw_obj}")      
        
def SwitchOff(robot, sw_obj):
    print(f"Attempting to switch off: {sw_obj}")
    robot_name = robot['name']
    agent_id = int(robot_name[-1]) - 1
    objs = list(set([obj["objectId"] for obj in c.last_event.metadata["objects"]]))
    ...
    if sw_obj_id is None:
        print(f"Error: Object '{sw_obj}' not found for switching off.")
        return
    
    action_queue.append({'action':'ToggleObjectOff', 'objectId':sw_obj_id, 'agent_id':agent_id})
    print(f"Successfully switched off: {sw_obj}")        
...
def SliceObject(robot, sw_obj):
    print(f"Attempting to slice: {sw_obj}")
    robot_name = robot['name']
    agent_id = int(robot_name[-1]) - 1
    objs = list(set([obj["objectId"] for obj in c.last_event.metadata["objects"]]))
    ...
    if sw_obj_id is None:
        print(f"Error: Object '{sw_obj}' not found for slicing.")
        return
    
    action_queue.append({'action':'SliceObject', 'objectId':sw_obj_id, 'agent_id':agent_id})
    print(f"Successfully sliced: {sw_obj}")      
  
def CleanObject(robot, sw_obj):
    print(f"Attempting to clean: {sw_obj}")
    robot_name = robot['name']
    agent_id = int(robot_name[-1]) - 1
    objs = list(set([obj["objectId"] for obj in c.last_event.metadata["objects"]]))
    ...
    if sw_obj_id is None:
        print(f"Error: Object '{sw_obj}' not found for cleaning.")
        return

    action_queue.append({'action':'CleanObject', 'objectId':sw_obj_id, 'agent_id':agent_id})
    print(f"Successfully cleaned: {sw_obj}") 
...
def parse_and_execute_task(robot):
    try:
        ...
        with open('logs/generated_function_name.json', 'r') as file:
            data = json.load(file)
            function_name = data["function_name"]
        
        # Dynamically import the module and reload it
        module = importlib.import_module('.task_functions', package='scripts')
...
def run_scripts():
    try:
        subprocess.run(['python', 'scripts/query_with_short_term_memory.py'], check=True)
        subprocess.run(['python', 'scripts/llm_as_planner.py'], check=True)
